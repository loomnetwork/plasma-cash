...
deposit1Utxo = event_data[0]['args']['slot']
...
coin = dan.get_plasma_coin(deposit1Utxo)
authority.submit_block()
dan.watch_exits(deposit1Utxo)
...
mallory_to_trudy = mallory.send_transaction(
    deposit1Utxo, trudy_to_mallory_block, trudy.token_contract.account.address
)
authority.submit_block()
mallory_to_trudy_block = authority.get_block_number()

# Trudy attemps to exit her illegitimate coin
trudy.start_exit(deposit1Utxo, trudy_to_mallory_block, mallory_to_trudy_block)
...
dan.start_exit(deposit1Utxo, 0, coin['deposit_block'])
dan.stop_watching_exits(deposit1Utxo)

increaseTime(w3, 8 * 24 * 3600)
authority.finalize_exits()

dan.withdraw(deposit1Utxo)
...
dan_balance_after = w3.eth.getBalance(dan.token_contract.account.address)
	...
	if !(dan_balance_before < dan_balance_after) {
...
print('dan has {} tokens'.format(danTokensEnd))
	if !(danTokensEnd == danTokensStart + 1) {
		log.Fatalf("END: dan has incorrect number of tokens-%d",dan_balance_after)
	}
