...
contract("Plasma ERC721 - Exit Spent Coin Challenge / `challengeAfter`", async function(accounts) {
    ...
    describe('Unit Tests', function() {
        ...
        it("Cannot provide an invalid signature", async function() {
            ...
            await txlib.exitAfterDeposit(plasma, UTXO, alice)

            let challengingTx = alice_to_bob.tx
            let sig = bob_to_charlie.sig // Sig must be from Alice
            let proof = tree_bob.createMerkleProof(UTXO.slot)
        ...
        it("Cannot provide an invalid merkle proof", async function() {
            ...
            let challengingTx = alice_to_bob.tx
            let sig = alice_to_bob.sig // Sig must be from Alice
        ...
        it("Cannot provide an invalid transaction", async function() {
            ...
            let challengingTx = bob_to_charlie.tx
            let sig = alice_to_bob.sig // Sig must be from Alice
            let proof = tree_bob.createMerkleProof(UTXO.slot)
    ...
    describe.only('C = Deposit, PC = Deposit', function() {
    ...
    describe('C = Non-Deposit, PC = Deposit', function() {
        it("Can challenge with a direct spend", async function() {
            ...
            // Alice exits at [0,UTXO.block]
            await txlib.exitAfter1Transfer(plasma, UTXO, bob)
